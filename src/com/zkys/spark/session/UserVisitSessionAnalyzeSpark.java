...
//		sc.checkpointFile("hdfs://");
		...
		/**
		 * 生成模拟测试数据
		 创建List<Row> rows = new ArrayList<Row>();
		 循环生成的数据添加到rows中
		 sc.parallelize(rows)生成RDD
		 由RDD转化成DataFrame，注册临时表
		 */
		 ...
		 *
		 * getActionRDDByDateRange方法返回指定时间段的session记录
		 * return actionDF.javaRDD();形式返回
		 */
...
//		return actionDF.javaRDD().repartition(1000);
		...
		return actionRDD.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Row>, String, Row>() {
			...
			public Iterable<Tuple2<String, Row>> call(Iterator<Row> iterator)
				...
				while(iterator.hasNext()) {
					...
					//随机生成的数据中，DataFrame中的sessionid是第3列，所以用row.getString(2)来取
	...
	private static JavaPairRDD<String, String> aggregateBySession(
		...
		// 对行为数据按session粒度进行分组
		//<sessionid,(sessionid,searchKeywords,clickCategoryIds)>
				...
				new PairFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple)
						...
						while(iterator.hasNext()) {
							...
							/**
                             * 计算session开始和结束时间
							 在groupbykey聚合session数据后,每个sessionid对应的操作都聚合在了一条
							 但是每个对应的操作时间顺序是混乱的,所以计算这个session对应最开始的操作
							 和最后的操作,时间相减就是这个session的时长
							 */
		...
		JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD.filter(
				...
				new Function<Tuple2<String,String>, Boolean>() {
					...
					public Boolean call(Tuple2<String, String> tuple) throws Exception {
						...
						// Constants.SESSION_COUNT就相当于累加器中的v2
...
//		});
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(  
		 ...
		 * sessionid2detailRDD行为数据flatMapToPair()得到
		 * Tuple2<Long, Long>(clickCategoryId, clickCategoryId)
		 * new Tuple2<Long, Long>(Long.valueOf(orderCategoryId),Long.valueOf(orderCategoryId))
		 * new Tuple2<Long, Long>(Long.valueOf(payCategoryId),Long.valueOf(payCategoryId))
		 */
